...
\caption{End-to-end lineage. Raw CSVs flow through seven pipeline
...
\item \texttt{run\_counterfactual(repo\_rate\_delta\_bps: int) -> dict}:
...
\item \texttt{get\_regime\_stats() -> dict}: per-regime descriptive
  statistics.
\item \texttt{search\_columns(query: str) -> list}: fuzzy-match
  against the column registry; supports synonyms (``inflation'' maps
  to the four CPI columns).
\item \texttt{query\_schema(family: str) -> list}: returns column
  metadata for a source family (rates, balance sheet, debt, etc.).
\item \texttt{get\_event\_context(start: str, end: str) -> list}:
  returns the curated NLP events that fall in the requested date
  range.
\end{itemize}
