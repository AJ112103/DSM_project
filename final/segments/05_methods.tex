...
rank 12 in the accommodation regime to rank 9 in the tightening
regime, and the absolute SHAP magnitude of the autoregressive lag
doubles from Tightening to Accommodation
(\texttt{target\_lag1}: 0.344 versus 0.689).}
\label{fig:shap-by-regime}
\end{figure}

Two substantive findings drop out of this comparison. First, the
absolute SHAP magnitude of every feature is roughly twice as large
in the accommodation regime than in the tightening regime
(\texttt{target\_lag1}'s SHAP value is 0.689 inside Regime 0 versus
0.344 inside Regime 1). Inside Accommodation, rates are stable,
recent history becomes the strongest single signal, and the model
effectively reduces to a feature-weighted AR(1) on WACMR with
\texttt{target\_lag1} carrying the bulk of the prediction. Inside
Tightening, no single feature dominates: predictions rely more
uniformly on the corridor rates and the autoregressive lag, with
smaller absolute contributions from each. Second, the Reverse Repo
(and post-2022 the SDF) Rate sits higher in the SHAP ranking inside
the tightening regime (rank 9) than inside the accommodation regime
(rank 12), reflecting the model's reliance on the explicit corridor
when active rate moves require closer week-to-week policy tracking.
